...
export const diff = (range, ranges, divider) => {
  const sorted = sort(ranges)
  const result = []
  let start = range[0]
  let end = null
  sorted.forEach(r => {
    ...
    start = start === range[0] ? start : start + divider
    end = end === range[1] ? end : end - divider
    if (r[0] - start > divider) result.push([start, end])
    start = r[1]
  })
  if (range[1] - start > divider) result.push([start + divider, range[1]])
  return result
}
...
export const calcFromVisitTimes = (visitTimes, event, waitingTime, duration) => {
  ...
  const newAvailableTimes = diff(range, ranges, waitingTime * 60000)
  return newAvailableTimes.filter(f => f[1] - f[0] >= duration * 60000).map(a => {
    return {
      startTime: new Date(a[0]),
      endTime: new Date(a[1])
